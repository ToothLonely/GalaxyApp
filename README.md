...
В ходе разработки приложения выяснилось, что API от NASA не работает (не предоставляет доступ запросам из России (скорее всего) или из-за шатдауна правительства США), однако было принято решение продолжить разработку.
